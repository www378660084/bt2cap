...
public class Crc {
	...
	public static boolean calcCrc(byte[] data){
		int crc = 0x0ffff;
		for(int i=0;i<data.length - 2;i++){
			crc = (crc >> 8) ^ crc_table[(crc&0xff)^(data[i]&0xFF)];
		}
		crc = abcsp_crc_reverse(crc);
		int num = (data[data.length-2]<<8)&0xFF00;
		num += data[data.length-1] & 0xFF;
		return crc == num;
	}
	
	private static int abcsp_crc_reverse(int crc){

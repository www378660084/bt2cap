...
public class UsbTxtProvider extends PacketProvider{
	...
	public UsbTxtProvider(InputStream inputStream) {
		super(inputStream);
		mRxBytes = new byte[0];
		mTxBytes = new byte[0];
	...
	private BtPacket handleBytes(){
		...
		if((bytes[0] == 0x05) && (mCurrentDirection == SEND)){
		...
		}else if((bytes[0] == 0x05) && (mCurrentDirection == RECV)){
			...
			Log.error("handle usb error unkown,type: " + bytes[0] + ",direction:"+mCurrentDirection);
			bytes = new byte[0];
		}
